...
exports.handler = async (event) => {
  ...
  if (!videoId) {
    return { statusCode: 200, headers, body: JSON.stringify({ title: 'Instagram Reel', thumbnail: '', platform: 'instagram' }) };
  }
  ...
  try {
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ title: data.title || 'YouTube Video', thumbnail, videoId, platform: 'youtube' })
    };
  } catch (e) {
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ title: 'YouTube Video', thumbnail, videoId, platform: 'youtube' })
    };
  }
};
